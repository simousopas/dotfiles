...
expected_hostname="macbookpro-9980hk"
...
source "$root_dir/etc/scripts/install-vscode-plugins.sh" \
	--silent --plugins-list "etc/vscode.plugins.txt"
...
log_info "\t >>> Configuring boot preferences and power management settings ..."
# sudo nvram AutoBoot=%00 -> Disables automatic boot when opening the lid.
# sudo nvram AutoBoot=%03 -> Restore original behavior.
sudo nvram AutoBoot=%00

# Settings when running on battery
sudo pmset -b displaysleep 3 sleep 5 gpuswitch 0 lidwake 1 powernap 0 \
...
sudo pmset -c displaysleep 10 sleep 10 gpuswitch 2 lidwake 1 powernap 1 \
